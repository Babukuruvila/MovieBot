...
async def __(_____, ______________):

    ____ = await _____._______(______________)  # type: ignore

    async def _________():
        ___ = temp.REQUESTERS.get(______________, {}).get("date", 0)
        ________ = await _____._____(  # type: ignore
            ______(
                peer=____,  # type: ignore
                offset_date=___,
                offset_user=_______(),  # type: ignore
                limit=100,
                requested=True,
            )
        )
        ___________ = (
            ________.importers[-1].date - 1 if len(________.importers) > 0 else ___
        )
        temp.REQUESTERS[______________] = {
            "list": temp.REQUESTERS.get(______________, {}).get("list", [])
            + [__________.user_id for __________ in ________.importers],
            "date": ___________,
        }
        return bool(________.importers)

    ____________ = await _________()
    while ____________:
        ____________ = await _________()
